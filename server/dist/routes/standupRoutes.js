...
router.get('/', requireUser(), async (req, res) => {
    try {
        ...
        const query = { teamId: currentUser.teamId };
...
// Response: { standups: Array<Standup> }
router.get('/range', requireUser(), async (req, res) => {
    try {
        const currentUser = req.user;
...
// Request: {}
// Response: { standups: Array<Standup> }
router.get('/team/:date', requireUser(), async (req, res) => {
    try {
        const currentUser = req.user;
...
router.post('/', requireUser(), async (req, res) => {
    try {
        ...
        try {
            const slackMessageId = await postStandupToSlack(currentUser.teamId, currentUser.name || currentUser.email, yesterdayWork || [], todayPlan || [], blockers || []);
            if (slackMessageId) {
...
router.put('/:id', requireUser(), async (req, res) => {
    try {
        ...
        if (yesterdayWork !== undefined)
            standup.yesterdayWork = yesterdayWork;
        if (todayPlan !== undefined)
            standup.todayPlan = todayPlan;
        ...
        if (standup.slackMessageId) {
            try {
                await updateSlackStandup(currentUser.teamId, standup.slackMessageId, currentUser.name || currentUser.email, standup.yesterdayWork, standup.todayPlan, standup.blockers);
            }
            catch (slackError) {
...
//# sourceMappingURL=standupRoutes.js.map