...
router.post('/invite', requireRole(ROLES.ADMIN), async (req, res) => {
    try {
...
// Request: { role: string }
// Response: { user: User }
router.put('/:id/role', requireRole(ROLES.ADMIN), async (req, res) => {
    try {
        const { id } = req.params;
        const { role } = req.body;
        const currentUser = req.user;
...
// Response: { user: User }
router.put('/:id/status', requireRole(ROLES.ADMIN), async (req, res) => {
    try {
        const { id } = req.params;
